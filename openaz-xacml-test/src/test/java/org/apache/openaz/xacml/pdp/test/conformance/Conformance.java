...
/**
 * Conformance is an application that runs a <code>ConformanceTestSet</code> and dumps results comparing the
 * actual and expected results. TO RUN in Eclipse: This is run as a Java Application. You must first create a
 * Run/Debug Configuration: Under the Argument tab, in Program Arguments you must set the -i or --input
...
public class Conformance {
    ...
    protected synchronized ConformanceScopeResolver getScopeResolver() {
        if (this.scopeResolver == null) {
            ...
            /*
             * TODO: Add the known scopes for the 2.0 conformance test. This could be made more general by
             * allowing loading from a properties file eventually.
             */
            try {
    ...
    private boolean init(String[] args) {
        ...
        for (int i = 0; i < args.length;) {
            ...
            if (args[i].equals("-i") || args[i].equals("--input")) {
                ...
                // A list of specific test names (e.g.: -t IIA001 IIA007 IIIE301) - default is to run all
                // tests
                ...
                // Include full details in the response, both the expected reqsponse (from file) and the
                // actual response
                ...
                i++;
                // When set, the XML must not contain extra attibutes/elements. Default is "lenient" where
                // unexpected entries are ignored
                ...
                // so to get an accurate number for how fast a single Request is handled we need to ignore the
                ...
                if (iterations < 1) {
                    System.err.println("Cannot use --iterations " + iterations
                                       + ".  Must use an integer greater than 0");
                    return false;
                }
            } else {
        ...
        this.testEngine = new ConformanceTestEngine(this.getScopeResolver(), lenientRequests,
    ...
    private void printHelp() {
        ...
        System.out.println("");
        System.out
            .println(" -f, --failures             Only include failed tests in the output.  \n"
        ...
        System.out.println("");
        System.out
            .println(" -i, --input <dir>  Directory containing the XML Request/Response files.  \n"
    ...
    private boolean failed(ConformanceTestResult conformanceTestResult) {
        ...
        if (!responseMatchResult.decisionsMatch() || !responseMatchResult.statusCodesMatch()) {
            ...
            if (!responseMatchResult.associatedAdviceMatches() || !responseMatchResult.attributesMatch()
                || !responseMatchResult.obligationsMatch() || !responseMatchResult.policyIdentifiersMatch()
    ...
    private void dump(Attribute attribute) {
        this.outputFileWriter.println("\t\t\t\t\tAttribute: "
                                      + (attribute.getAttributeId() == null ? "" : attribute.getAttributeId()
                                          .stringValue()));
    ...
    private void dump(AttributeCategory attributeCategory) {
        this.outputFileWriter.println("\t\t\tAttributeCategory: "
                                      + (attributeCategory.getCategory() == null ? "" : attributeCategory
    ...
    private void dump(Result result) {
        ...
        this.outputFileWriter
            .println("\t\tDecision: " + (result.getDecision() == null ? "null" : result.getDecision().name()));
        if (result.getStatus() == null) {
            ...
            if (result.getStatus().getStatusMessage() != null) {
                this.outputFileWriter
                    .println("\t\t\tStatusMessage: " + result.getStatus().getStatusMessage());
            }
