...
/**
 * Test JSON Request convert to object - Conformance tests TO RUN - use jUnit In Eclipse select this file or
 * the enclosing directory, right-click and select Run As/JUnit Test NOTE: The "correct" way to verify that
 * each JSON string gets translated into our internal Objects correctly is to look explicitly at each of the
 * child objects and verify that they are correct. This would involve a lot of coding to get child of child of
 * child and individually verify each property of each element. To simplify testing we assume that
    ...
    public void testConformanceRequests() {
        ...
        try {
            for (File f : filesInDirectory) {
                ...
                // jsonString =
                // "{\"Request\":{\"Category\":[{\"CategoryId\":\"urn:oasis:names:tc:xacml:1.0:subject-category:access-subject\",\"Attribute\":["
                // +
                ...
                assertEquals("File '" + currentFile.getName() + "' CombinedDecision",
                             xmlRequest.getCombinedDecision(), jsonRequest.getCombinedDecision());
                assertEquals("File '" + currentFile.getName() + "' getReturnPolicyIdList",
                             xmlRequest.getReturnPolicyIdList(), jsonRequest.getReturnPolicyIdList());
                assertEquals("File '" + currentFile.getName() + "' requestDefaults",
                             xmlRequest.getRequestDefaults(), jsonRequest.getRequestDefaults());
                ...
                if (jsonCollection.size() != xmlCollection.size()) {
                    ...
                    errorMessage = "File '" + currentFile.getName()
                                   + "' MultiRequests have different contents.  ";
                }
                if (errorMessage != null) {
                    ...
                    fail(errorMessage + "\nXML(" + xmlCollection.size() + ")='" + xmlContents + "'  \nJSON("
                         + jsonCollection.size() + ")='" + jsonContents + "'" + "\njson='" + jsonString + "'");
                }
                ...
                if (jsonAttrCollection.size() != xmlAttrCollection.size()) {
                    ...
                    errorMessage = "File '" + currentFile.getName()
                                   + "' RequestAttributes have different contents.  JSON is missing attr="
                                   + attrName;
                }
                if (errorMessage != null) {
                    String xmlContents = "";
                    ...
                    while (rrIt.hasNext()) {
                        ...
                        if (ras.getContentRoot() != null) {
                            ...
                            try {
                                ...
                                transformer.transform(new DOMSource(ras.getContentRoot()),
