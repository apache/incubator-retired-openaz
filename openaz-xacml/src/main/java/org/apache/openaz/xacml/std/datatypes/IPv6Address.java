...
public class IPv6Address extends IPAddress {
    ...
    // They are usually represented differently (4 octets vs. one number), but they are actually equivalent
    // since they are both specifying which bits
    // in the address are defining a set of subnets. The trick is that IPv4 is expected to start with the
    // top-most bit and fill in to the right,
    // and the prefix says how many of those bits have been set to 1. Typical examples:
    // 255.0.0.0 = 8
    // 255.255.0.0 = 16
    // 255.255.255.0 = 24
    // Thus it is possible to hold the addressMask for IPv4 internally as a single number (as IPv6 does) and
    // translate its representation to/from the octet version
    // as input string is parsed and output string generated, which might allow for common processing between
    // IPv4 and IPv6.
    ...
    public IPv6Address(short[] addressBytesIn, Short prefixIn, PortRange portRangeIn) {
        ...
        if (addressBytesIn.length != 8) {
            throw new IllegalArgumentException("Invalid byte count for IPv6 address: "
                                               + addressBytesIn.length);
        }
    }
    ...
    /**
     * Generate a string representing just the address part without the [], prefix or portrange. This accounts
     * for the case of an IPv4-form address being represented in IPv6 format.
     *
     * @param addressParts
     * @return
     */
    ...
    /**
     * Parses a string that can represent an IPv6 address into an array of 8 shorts representing the 8 16-bit
     * components of the address. Allows for the IPv4 equivalence address: ::FFFF:w.x.y.z where the 32 bits of
     * the IPv4 address are encoded into the lower two shorts of the IPv6 address
     *
     * @param ipv6Address
     * @return
    ...
    protected static short[] getAddress(String ipv6Address) throws ParseException {
        ...
        if (addressFields.length == 3 && addressFields[0].length() == 0
            && addressFields[1].equalsIgnoreCase("FFFF") && addressFields[2].contains(".")) {
            isIPv4 = true;
        }
        ...
        for (int i = 0; i < addressFields.length; i++) {
            if (addressFields[i].length() == 0) {
                 ...
                 */
                if (sawMissingField) {
                    throw new ParseException("Invalid IPv6Address string \"" + ipv6Address
                                             + "\": multiple zero runs", i);
                }
                ...
                try {
                    ...
                    throw new ParseException("Invalid IPv6Address component \"" + addressFields[i]
                                             + "\": invalid hex", i);
                }
            }
        }
    ...
    /**
     * Specs not clear on format of a "prefix". It could be either "[" <address> [ "/" <prefix> ] "]" [ ":"
     * portrange ] or "[" <address> "]" [ "/" <prefix> ] [ ":" portrange ] but we cannot tell which the specs
     * are saying. To avoid problems in the future it seems safest to implement both of these formats.
     *
    ...
    public static IPv6Address newIPv6Instance(String ipv6AddressString) throws ParseException {
        ...
        if (ipv6AddressString == null || ipv6AddressString.length() == 0) {
            ...
            throw new ParseException("Invalid IPv6Address string \"" + ipv6AddressString
                                     + "\": missing opening bracket", 0);
        }
        ...
        /*
         * See if there is a slash for the prefix The four cases we are looking at are: ] ]/<prefix>
         * /<prefix>] all of which may be followed by ":<portrange>"
         */
        // adjust starting point for case of ]/<prefix>
        Short prefix = null;
        if (nextPos < ipv6AddressString.length() - 1 && ipv6AddressString.charAt(nextPos) == ']'
            && ipv6AddressString.charAt(nextPos + 1) == '/') {
            nextPos++;
        }
        if (nextPos < ipv6AddressString.length() && ipv6AddressString.charAt(nextPos) == '/') {
            nextPos++;
            if (nextPos >= ipv6AddressString.length() || ipv6AddressString.charAt(nextPos) == ']'
                || ipv6AddressString.charAt(nextPos) == ':') {
                throw new ParseException("Invalid Ipv6Address string \"" + ipv6AddressString
                                         + "\": prefix designation without value", nextPos);
            }
            ...
            if (prefix > 128) {
                throw new ParseException("Invalid Ipv6Address string \"" + ipv6AddressString
                                         + "\": prefix is larger than 128", nextPos);
            }
            nextPos = addressEnd;
        }
        // adjust for case of
        ...
        if (nextPos < ipv6AddressString.length() && ipv6AddressString.charAt(nextPos) == ':') {
            if (ipv6AddressString.substring(nextPos + 1).length() < 1) {
                throw new ParseException("Invalid IPv6 address string \"" + ipv6AddressString
                                         + "\": no portrange given after ':'", nextPos + 1);
            }
        ...
        if (nextPos < ipv6AddressString.length()) {
            throw new ParseException("Invalid Ipv6Address string \"" + ipv6AddressString
                                     + "\": unknown content at end", nextPos);
        }
    ...
    /**
     * Guess whether or not the given string is intended to be an IPv6 address. Because there may be all kinds
     * of errors in the representation, the only way to know that it is a legal IPv6 address is to fully parse
     * it, and if the parse fails then we only know that is is not a legal IPv6 address (it might be a legal
     * IPv4 or other address). We could just have the IPAddress.newInstance method call the full parser and do
     * some quick checks there. The disadvantage of that is that we would need to set up a try/catch
     * (expensive) and in many cases there would be unneeded method calls involved. So we take a guess here.
     *
    ...
    /**
     * The specifications are ambiguous on how to handle /prefix in IPv6. There are two possibilities and the
     * documents are silent on how to handle it, and no examples are readily apparent on the internet. The two
     * possible representations are: [<address>/prefix] and [<address>]/prefix The prefix is optional as is
     * the ":&lt;portrange&gt;" that can follow it. We have chosen to use the first representation because the
     * prefix seems to be part of the address itself, which is supposed to be inside the brackets.
     */
    @Override
