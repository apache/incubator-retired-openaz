...
public class IPv4Address extends IPAddress {
    ...
    // 255.255.0.0 = 16
    // 255.255.255.0 = 24
    // Thus it is possible to hold the addressMask for IPv4 internally as a single number and translate its
    // representation to/from the octet version
    ...
    public IPv4Address(short[] addressBytesIn, short[] addressMaskIn, PortRange portRangeIn) {
        ...
        if (addressBytesIn.length != 4) {
            throw new IllegalArgumentException("Invalid byte count for IPv4 address: "
        ...
        for (int i = 0; i < 4; i++) {
            try {
                ...
                if (octet < 0 || octet > 255) {
                    throw new ParseException("Invalid IPv4 address string \"" + ipv4AddressString
                                             + "\": invalid octet: \"" + addressParts[i], 0);
                } else {
    ...
    public static IPv4Address newIPv4Instance(String ipv4AddressString) throws ParseException {
        ...
        if (colonPos >= 0) {
            if (ipv4AddressString.substring(colonPos + 1).length() < 1) {
                throw new ParseException("Invalid IPv4 address string \"" + ipv4AddressString
                                         + "\": no portrange given after ':'", colonPos + 1);
            }
    ...
    /**
     * Guess whether or not the given string is intended to be an IPv4 address. Because there may be all kinds
     * of errors in the representation, the only way to know that it is a legal IPv4 address is to fully parse
     * it, and if the parse fails then we only know that is is not a legal IPv4 address (it might be a legal
     * IPv6 or other address). We could just have the IPAddress.newInstance method call the full parser and do
