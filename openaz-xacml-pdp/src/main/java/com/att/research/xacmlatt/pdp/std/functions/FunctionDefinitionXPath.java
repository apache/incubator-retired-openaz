...
/**
 * FunctionDefinitionXPath extends
 * {@link com.att.research.xacmlatt.pdp.std.functions.FunctionDefinitionHomogeneousSimple} to implement the
 * XACML XPath predicates as functions taking one or two <code>XPathExpression</code> arguments and returning
 * either an <code>Integer</code> or a <code>Boolean</code>. XACML version 1.0 and 2.0 used
 * <code>String</code> data type as input. We do NOT support those functions because of ambiguity in the
 * meaning of those Strings. The root of the XPath changed from the Request level in 2.0 to the Content level
 * in 3.0. Also the 2.0 Requests contain only one Content, located in the resources category, while 3.0 allows
 * Requests to contain Content in multiple categories. In the first implementation of XACML we had separate
 * files for each XACML Function. This release combines multiple Functions in fewer files to minimize code
 * duplication. This file supports the following XACML codes: xpath-node-count xpath-node-equals
 * xpath-node-match
 *
