 ...
 * code duplication. This file supports the following XACML codes: string-one-and-only boolean-one-and-only
 * integer-one-and-only double-one-and-only time-one-and-only date-one-and-only dateTime-one-and-only
 * anyURI-one-and-only hexBinary-one-and-only base64Binary-one-and-only dayTimeDuration-one-and-only (version
 * 1 and3) yearMonthDuration-one-and-only (version 1 and 3) x500Name-one-and-only rfc822Name-one-and-only
 * ipAddress-one-and-only dnsName-one-and-only
 *
...
public class FunctionDefinitionBagOneAndOnly<I> extends FunctionDefinitionBase<I, I> {
    ...
    public ExpressionResult evaluate(EvaluationContext evaluationContext, List<FunctionArgument> arguments) {
        ...
        if (bag.size() != 1) {
            return ExpressionResult.newError(new StdStatus(StdStatusCode.STATUS_CODE_PROCESSING_ERROR,
                                                           getShortFunctionId() + " Expected 1 but Bag has "
                                                               + bag.size() + " elements"));
        }
            ...
            return ExpressionResult
                .newError(new StdStatus(StdStatusCode.STATUS_CODE_PROCESSING_ERROR,
                                        getShortFunctionId()
                                            ...
                                            + " got "
                                            + this.getShortDataTypeId(attributeValueOneAndOnly
                                                .getDataTypeId())));
        }
