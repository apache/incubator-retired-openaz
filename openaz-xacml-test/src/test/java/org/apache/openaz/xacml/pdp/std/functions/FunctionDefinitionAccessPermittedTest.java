 ...
 * the enclosing directory, right-click and select Run As/JUnit Test NOT
 * IMPLEMENTED!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! This
 * function is not yet implemented so these tests intentionally fail.
 */
public class FunctionDefinitionAccessPermittedTest {
    ...
    String reqStrMdRecordSimpson = "<md:record>"
                                   + "<md:hospital_info>"
    ...
    String reqStrMalformedContent = " <Content>" + "<md:record>" + "<md:hospital_info>"
                                    ...
                                    + "</Content>";
    String reqStrMdRecordSpringer = "<md:record>" + "<md:hospital_info>" + "<md:name>XYZ Hospital</md:name>"
                                    + "<md:department>Surgery</md:department>" + "</md:hospital_info>"
                                    + "<md:patient_info>" + "<md:name>Jerry Springer</md:name>"
                                    + "<md:age>65</md:age>" + "<md:sex>male</md:sex>"
                                    + "<md:health_insurance>765432</md:health_insurance>"
                                    + "</md:patient_info>" + "<md:diagnosis_info>" + "<md:diagnosis>"
                                    ...
                                    + "</md:diagnosis>" + "<md:pathological_diagnosis>" + "<md:diagnosis>"
                                    + "<md:item type=\"primary\">We have no idea</md:item>"
                                    + "</md:diagnosis>" + "<md:date>2012-07-22</md:date>"
                                    + "<md:malignancy type=\"no\"/>" + "</md:pathological_diagnosis>"
                                    + "</md:diagnosis_info>" + " </md:record>";
    String reqStrContentMdRecordSpringer = "<Content>" + reqStrMdRecordSpringer + "</Content>";
    ...
    public FunctionDefinitionAccessPermittedTest() {
        try {
            ...
            attrUriResources = new FunctionArgumentAttributeValue(
                                                                  DataTypes.DT_ANYURI
                                                                      .createAttributeValue("urn:oasis:names:tc:xacml:3.0:attribute-category:resource"));
            attrUriAction = new FunctionArgumentAttributeValue(
                                                               DataTypes.DT_ANYURI
                                                                   .createAttributeValue("urn:oasis:names:tc:xacml:3.0:attribute-category:action"));
            attrUriNotInRequest = new FunctionArgumentAttributeValue(
                                                                     DataTypes.DT_ANYURI
                                                                         .createAttributeValue("NoSuchURI"));
            attrUriNotCategory = new FunctionArgumentAttributeValue(
                                                                    DataTypes.DT_ANYURI
                                                                        .createAttributeValue("urn:oasis:names:tc:xacml:1.0:resource:resource-id"));
            ...
            attrXSimpson = new FunctionArgumentAttributeValue(
                                                              DataTypes.DT_STRING
                                                                  .createAttributeValue(reqStrMdRecordSimpson));
            attrXSpringer = new FunctionArgumentAttributeValue(
                                                               DataTypes.DT_STRING
                                                                   .createAttributeValue(reqStrMdRecordSpringer));
            attrXContentSimpson = new FunctionArgumentAttributeValue(
                                                                     DataTypes.DT_STRING
                                                                         .createAttributeValue(reqStrContentMdRecordSimpson));
            attrXContentSpringer = new FunctionArgumentAttributeValue(
                                                                      DataTypes.DT_STRING
                                                                          .createAttributeValue(reqStrContentMdRecordSpringer));
            attrXBadXML = new FunctionArgumentAttributeValue(
                                                             DataTypes.DT_STRING
                                                                 .createAttributeValue(reqStrMalformedContent));
    ...
    public void testAccess_permitted() {
        ...
        assertEquals("function:access-permitted Parsing of XML string failed.  Cause='The element type \"md:hospital_info\" must be terminated by the matching end-tag \"</md:hospital_info>\".'",
        ...
        res = fd.evaluate(null, arguments);
        assertFalse(res.getStatus().isOk());
        assertEquals("function:access-permitted First argument must be a urn for an attribute-category, not 'urn:oasis:names:tc:xacml:1.0:resource:resource-id",
                     res.getStatus().getStatusMessage());
        assertEquals("urn:oasis:names:tc:xacml:1.0:status:syntax-error", res.getStatus().getStatusCode()
            .getStatusCodeValue().stringValue());
        ...
        assertFalse(res.getStatus().isOk());
        assertEquals("function:access-permitted Got null Request in EvaluationContext", res.getStatus()
            .getStatusMessage());
        ...
        res = fd.evaluate(new StdEvaluationContext(requestMdRecord, null, null), arguments);
        assertFalse(res.getStatus().isOk());
        assertEquals("function:access-permitted Expected data type 'anyURI' saw 'string'", res.getStatus()
            .getStatusMessage());
        assertEquals("urn:oasis:names:tc:xacml:1.0:status:processing-error", res.getStatus().getStatusCode()
            .getStatusCodeValue().stringValue());
        ...
        res = fd.evaluate(new StdEvaluationContext(requestMdRecord, null, null), arguments);
        assertFalse(res.getStatus().isOk());
        assertEquals("function:access-permitted Expected data type 'string' saw 'anyURI'", res.getStatus()
            .getStatusMessage());
        assertEquals("urn:oasis:names:tc:xacml:1.0:status:processing-error", res.getStatus().getStatusCode()
            .getStatusCodeValue().stringValue());
        ...
        res = fd.evaluate(new StdEvaluationContext(requestMdRecord, null, null), arguments);
        assertFalse(res.getStatus().isOk());
        assertEquals("function:access-permitted Expected 2 arguments, got 0", res.getStatus()
            .getStatusMessage());
        assertEquals("urn:oasis:names:tc:xacml:1.0:status:processing-error", res.getStatus().getStatusCode()
            .getStatusCodeValue().stringValue());
        ...
        res = fd.evaluate(new StdEvaluationContext(requestMdRecord, null, null), arguments);
        assertFalse(res.getStatus().isOk());
        assertEquals("function:access-permitted Expected 2 arguments, got 1", res.getStatus()
            .getStatusMessage());
        assertEquals("urn:oasis:names:tc:xacml:1.0:status:processing-error", res.getStatus().getStatusCode()
            .getStatusCodeValue().stringValue());
        ...
        res = fd.evaluate(new StdEvaluationContext(requestMdRecord, null, null), arguments);
        assertFalse(res.getStatus().isOk());
        assertEquals("function:access-permitted Expected 2 arguments, got 3", res.getStatus()
            .getStatusMessage());
        assertEquals("urn:oasis:names:tc:xacml:1.0:status:processing-error", res.getStatus().getStatusCode()
            .getStatusCodeValue().stringValue());
